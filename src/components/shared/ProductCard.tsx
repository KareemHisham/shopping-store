...
import { CsButton } from "../Index.ts";

const ProductCard = ({ item }: { item: IProduct }) => {
    return (
        <div className='bg-[#F4F5F7] relative group overflow-hidden flex flex-col h-full'>
            <img src={item.image} alt={item.title} width={285} height={300} loading='lazy' className='object-cover transition-transform duration-700 group-hover:scale-105' />
            ...
            <div className="absolute right-5 top-2 flex gap-2">
                ...
                {item?.is_new && <span className='w-10 h-10 bg-[#2EC1AC] text-white rounded-full text-[10px] flex items-center justify-center'>New</span>}
            ...
            <div className="absolute inset-0 bg-[#3a3a3ab3] flex items-center justify-center gap-3 invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all duration-75">
                <CsButton type="button" classes="text-primary bg-slate-50 py-2 px-4 rounded-md border-primary border text-xs transition-transform duration-200 -translate-y-7 group-hover:translate-y-0">
                    <FaCartPlus size={20} />
                </CsButton>
